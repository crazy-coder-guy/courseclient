...
`}</style>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {course.learning_objectives?.split(',').map((objective, index) => (
              <div key={index} className="flex items-start gap-2">
                <div className="w-5 h-5 text-purple-600 mt-0.5 flex-shrink-0">
                ...
                <span className="text-gray-600 text-sm font-normal">{objective.trim()}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  >
    ...
    <span className="relative z-10 flex items-center justify-center gap-2 drop-shadow-sm">
      {hasPurchased ? 'Start Learning' : (
        <>
          Buy Now
          {/* Inline SVG icon with hover/click animations */}
          <svg
