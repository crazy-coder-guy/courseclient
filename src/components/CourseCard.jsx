...
export default function CourseCard({ course, isLoading = false }) {
  ...
  const [isWishlistInitialized, setIsWishlistInitialized] = useState(false); // NEW: track init
  ...
  useEffect(() => {
    if (course?.id) {
      const storedWishlist = JSON.parse(localStorage.getItem('wishlistCourses') || '{}');
      setIsWishlisted(Boolean(storedWishlist[course.id]));
      setIsWishlistInitialized(true); // mark wishlist as loaded from storage
    }
  }, [course?.id]);

  // Save wishlist status to localStorage whenever it changes (only after init)
  useEffect(() => {
    if (!isWishlistInitialized || !course?.id) return; // skip until initialized

    const storedWishlist = JSON.parse(localStorage.getItem('wishlistCourses') || '{}');
    if (isWishlisted) {
      storedWishlist[course.id] = true;
    } else {
      delete storedWishlist[course.id];
    }
    localStorage.setItem('wishlistCourses', JSON.stringify(storedWishlist));
  }, [isWishlisted, course?.id, isWishlistInitialized]);
